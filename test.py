...
def transition_model(corpus, page, damping_factor):
    ...
    """
    # create empty dict to fill with values
    trans_mod = {}
    ...
    while changes >= 0.001:
        ...
        for page in iterate_PR:
            ...
            if len(parents) != 0 :
                ...
                for page in corpus:
                    # act aws if one link from every page in the corpus
                    num_links = len(corpus[page])
                    if num_links != 0 :
                        val = previous_state[page] / num_links
                    else:
                        val = previous_state[page]
                    second.append(val)
            ...
            if changes < new_change:
                changes = new_change
        iterations += 1
    print(f'PageRank value stable after {iterations} iterations.')
    return iterate_PR
